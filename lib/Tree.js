...
export class Tree{
    ...
    constructor(
        ...
        for (let i = 0; i < logLines.length; i++) {
            ...
            obj.data.text = `${obj.data.value.subEvt} ${obj.data.value.psPath}`;

            // 視覚系の情報を記録
            obj.view.colorId = (() => {
                switch (obj.data.value.evt) {
                    case "ps":
                        switch (obj.data.value.subEvt) {
                            case "inject":
                            case "guardInject":
                                return 1;
                        }
            ...
            for (let i = 0; i < obj.data.text.length; i++) {
                textLength += font.data.glyphs[obj.data.text[i]].ha;
            }
            obj.view.textLength = textLength / font.data.resolution * this.fontSize;
    ...
    updateTree(){
        ...
        (data, depthes) => {
            if(data.tree.parent && data.tree.variable) data.tree.parent.tree.child.push(data);
