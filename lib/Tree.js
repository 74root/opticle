                        ...
                        switch (v.subEvt) {
                            case 'close':
                                return `${v.subEvt} ${v.path}` +
                                    (v.size ? ` filesize=${parseInt(v.size).toLocaleString()}` : '') +
                                    ` readsize=${parseInt(v.read).toLocaleString()}` +
                                    ` writesize=${parseInt(v.write).toLocaleString()}`;
                            case 'copy':
                            case 'rename':
                                return `${v.subEvt} ${v.path} to ${v.dstPath}`;
                            default:
                                return `${v.subEvt} ${v.path}`
                        }
                    case 'reg':
