...
class DrawTree {
    ...
    constructor(tree, parent = null, depth = 0, ithSibl = 1) {
        ...
        this.children = [];
        for (let i = 0; i < tree.tree.child.length; i++) this.children.push(
            new DrawTree(tree.tree.child[i], this, depth + 1, i + 1)
        );
        this.parent = parent;
        this.thread = null;
        this.mod = 0;
    ...
    left() {
        return this.thread ||
            this.children.length > 0 ? this.children[0] : null;
    }
    right() {
        return this.thread ||
...
const second_walk = async (v, m = 0, depth = 0) => {
    ...
    v.y = depth;
    // await Promise.all([
    //     ...Array(v.children.length).map(
    //         (x, i) => second_walk(v.children[i], m + v.mod, depth + 1)
    //         )
    // ]);
    for (let i = 0; i < v.children.length; i++)
        second_walk(v.children[i], m + v.mod, depth + 1);
    return 0;
};
...
const ancestor = (vil, v, default_ancestor) => {
    return v.parent.children.findIndex(x => x === vil.ancestor) !== -1 ?
