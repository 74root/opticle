...
const apportion = (vertex, default_ancestor, distance) => {
    ...
    while (vertex_inside_left.right() !== null && vertex_inside_right.left() !== null) {
        ...
        // shiftは2つの木の内側の差分
        const shift = (vertex_inside_left.x + shift_inside_left) - (vertex_inside_right.x + shift_inside_right) + distance;
...
const firstwalk = (v, distance = 1) => {
    ...
    if (v.children.length == 0) {
        // 葉であれば, 兄弟の隣に配置するだけ
        v.x = v.leftmost_sibling !== null ? (v.lbrother().x + distance) : 0;
    }
    else {
        // 子があった場合
