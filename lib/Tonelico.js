...
`,       
        ...
        const group = tree.dfsData(undefined, (node, value = []) => {
            ...
            edgeGeometry.addAttribute('position', new THREE.BufferAttribute(Float32Array.from((function* gen() {
            ...
            nodeGeometry.addAttribute('nodeCoord', new THREE.InstancedBufferAttribute(Uint32Array.from((function* gen() {
                for(let i = 0; i < node.tree.child.length; i++){
                    yield node.tree.child[i].tree.brtNode.y;
                    yield node.tree.child[i].tree.brtNode.x;
                    yield 0;
            ...
            for(let i = 0; i < node.tree.child.length; i++){
                ...
                // const textShape = font.generateShapes(node.tree.child[i].data.text, 2);
                const textShape = font.generateShapes(`${node.tree.child[i].tree.variable}`, 2);
                const bgAppend = new THREE.ShapeBufferGeometry(textShape , 2 );
                bgAppend.translate(node.tree.child[i].tree.brtNode.y, node.tree.child[i].tree.brtNode.x - 10, 0);
                ...
                if(textGeometry.attributes.position.array.length > 0x20000 || i + 1 == node.tree.child.length){
                    // console.log(`generating text vertices... progress: ${i * 100 / tree.data.length} %`);
                    const textMesh = new THREE.Mesh( textGeometry, edgeMaterial );
                    group.add(textMesh);
                    textGeometry = null;
                }
            }
