...
class Camera{
    constructor(canvas){
        this._camera = new THREE.OrthographicCamera(-window.innerWidth/2, window.innerWidth/2, window.innerHeight/2, -window.innerHeight/2, 1, 2000);
...
export class Tonelico{
    ...
    async init(tree = this.tree){
        ...
        buchheim(tree.root, 5);
        tree.foreachData(node => {
            // scale tree
            node.tree.brtNode.y *= ( tree.maxTextLength + 6 ) * 1.25;

        });
        tree.dfsData(undefined, node => {
            if(node.tree.parent == null){
                node.tree.brtNode.x = 0;
                node.tree.brtNode.y = 0;
            }
            else{
                node.tree.brtNode.x -= node.tree.parent ? node.tree.parent.tree.brtNode.x : 0;
                node.tree.brtNode.y -= node.tree.parent ? node.tree.parent.tree.brtNode.y : 0;    
            }
        });
...
`,       
        ...
        const nodeBackMaterial = new THREE.RawShaderMaterial({
            side: THREE.DoubleSide,
            // depthTest: false,
            // depthWrite: false,
            uniforms: {
...
void main(){
    vec3 pos = position;
    pos.x += xMorph * nodeLength;
    colorV = texture(colorMap, vec2(0.5, 0.125) * (colorId + 0.5) ) * 0.125;
    gl_Position = projectionMatrix * modelViewMatrix * vec4(pos + nodeCoord, 1.0);
}
`,
            fragmentShader:             
...
`,       
        ...
        const _group = tree.dfsData(undefined, (node, childGroups = []) => {

            if(node.tree.child.length === 0) return null;

            // console.log(value);

            const nodeRelativeX = ( tree.maxTextLength + 6 ) * 1.25 
                - (node.tree.parent === null ? 0 : node.tree.parent.view.textLength + 6);

            const group = new THREE.Group();
            this._groups.push(group);
            for (let i = 0; i < childGroups.length; i++) {
                if(childGroups[i] == null) continue; 
                const child = node.tree.child[i];
                childGroups[i].position.x = nodeRelativeX + child.view.textLength + 6;
                childGroups[i].position.y = child.tree.brtNode.x;
                group.add(childGroups[i]);
            }
            ...
            const nodeCoordBuffer = new THREE.InstancedBufferAttribute(Float32Array.from((function* gen() {
                for(let i = 0; i < node.tree.child.length; i++){
                    yield nodeRelativeX;
            ...
            nodeGeometry.addAttribute('nodeCoord', nodeCoordBuffer);

            const xMorphBuffer = new THREE.BufferAttribute(new Float32Array([
                0, 0, 0, 1, 1, 1
            ]), 1);
            nodeGeometry.addAttribute('xMorph', xMorphBuffer);

            const textLengthBuffer = new THREE.InstancedBufferAttribute(Float32Array.from((function* gen() {
                for(let i = 0; i < node.tree.child.length; i++){
                    yield node.tree.child[i].view.textLength + 6;
                }
            })()), 1);
            nodeGeometry.addAttribute('nodeLength', textLengthBuffer);
        
            const colorIdBuffer = new THREE.InstancedBufferAttribute(Float32Array.from((function* gen() {
                for(let i = 0; i < node.tree.child.length; i++){
                    yield node.tree.child[i].view.colorId;
                    // yield 3.0;
                }
            })()), 1);
            nodeGeometry.addAttribute('colorId', colorIdBuffer);
            ...
            nodeBackGeometry.addAttribute('nodeCoord', nodeCoordBuffer);
            nodeBackGeometry.addAttribute('nodeLength', textLengthBuffer);
            nodeBackGeometry.addAttribute('colorId', colorIdBuffer);
            nodeBackGeometry.addAttribute('xMorph', xMorphBuffer);

            const nodeBackMesh = new THREE.Mesh(nodeBackGeometry, nodeBackMaterial);
            nodeBackMesh.frustumCulled = false;


            edgeMesh.position.z = -0.4;
            edgeOutMesh.position.z = -0.3;
            nodeBackMesh.position.z = -0.2;
            nodeMesh.position.z = -0.1;
            group.add(edgeMesh);
        ...
        if(_group != null && _group.isObject3D === true) this._scene.add(_group);

        this.treeObject = _group;
