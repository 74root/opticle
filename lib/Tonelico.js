...
export class Tonelico{
    ...
    async init(tree = this.tree){
        ...
        tree.foreachData(node => {
            // scale tree
            node.tree.y *= ( tree.maxTextLength + 6 ) * 1.25;
        ...
        tree.dfsData(undefined, node => {
            if(node.tree.parent == null){
                ...
                node.tree.relX = node.tree.x - node.tree.parent.tree.x;
                node.tree.relY = node.tree.y - node.tree.parent.tree.y;    
            }
        });
...
`,       
        ...
        const _group = tree.dfsData(undefined, (node, childGroups = []) => {
            ...
            for (let i = 0; i < childGroups.length; i++) {
                ...
                childGroups[i].position.y = child.tree.relX;
            ...
            const nodeCoordBuffer = new THREE.InstancedBufferAttribute(Float32Array.from((function* gen() {
                for(let i = 0; i < node.tree.child.length; i++){
                    yield nodeRelativeX;
                    yield node.tree.child[i].tree.relX;
                    yield 0;
                }
            })()), 3);
            ...
            for(let i = 0; i < node.tree.child.length; i++){
                ...
                for (let j = 0; j < coordTA.length / 3; j++) {
                    ...
                    coordTA[j * 3 + 1] += child.tree.relX - 1;
                }
